#!/usr/bin/env python3
"""Qwen3.6 One-Token Correctness Oracle.
Prefills the prompt, then decodes exactly one token and checks logits, entropy, and MoE selections.
"""
import argparse
import ctypes
import numpy as np
...
def main():
    parser = argparse.ArgumentParser(description="Qwen3.6 One-Token Correctness Oracle")
    ...
    parser.add_argument("--compare", default=None, help="Compare current run against specified golden JSON")
    parser.add_argument("--compare-golden", action="store_true", help="Compare current run against golden registry entry")
    parser.add_argument("--golden-name", default="exact_good_1token", help="Golden registry entry name to save or compare")
    ...
    # Decode next token (greedy or temperature-based)
    # The last logit indices/values correspond to the first decode token distribution
    order = np.argsort(values[:k])[::-1]
    next_token = int(indices[:k][order][0])
    
    # 2. Reset MoE stats right before decoding the 1 token to isolate its stats
    if hasattr(lib, "lko_runner_reset_moe_stats"):
        lib.lko_runner_reset_moe_stats()
    ...
    print(f"Decoding 1 token (token_id={next_token}, pos={pos})...")
    if has_entropy_api:
        k = lib.lko_runner_step_with_entropy(
            next_token, pos, pos+1, hn.ctypes.data, 50,
            indices.ctypes.data, values.ctypes.data, ctypes.byref(entropy_val)
        )
        decode_entropy = float(entropy_val.value)
    else:
        k = lib.lko_runner_step(
            next_token, pos, pos+1, hn.ctypes.data, 50,
            indices.ctypes.data, values.ctypes.data
        ...
        probs /= np.sum(probs)
        decode_entropy = -float(np.sum(probs * np.log(probs + 1e-10)))

    total_time_ms = (time.perf_counter() - t0) * 1000.0

    decode_order = np.argsort(values[:k])[::-1]
    decode_top10_ids = [int(x) for x in indices[:k][decode_order][:10]]
    decode_top10_values = [float(x) for x in values[:k][decode_order][:10]]
    decode_top10_tokens = [tok.decode([x]) for x in decode_top10_ids]
    decode_hidden_norm = float(np.linalg.norm(hn))
    decoded_token = decode_top10_ids[0]

    # Retrieve expert stats for this decode step
    selected_expert_stats = {}
    if hasattr(lib, "lko_runner_get_moe_stats_json"):
        ...
        if stats_ptr:
            ...
            for idx, lyr_data in enumerate(stats_json.get("layers", [])):
                selected_expert_stats[str(idx)] = {
                    "last_selected_ids": lyr_data.get("last_selected_ids", []),
                    "last_selected_weights": lyr_data.get("last_selected_weights", [])
                }

    print("\nDecode step results:")
    print(f"  Decoded token ID: {decoded_token} ({repr(tok.decode([decoded_token]))})")
    print(f"  Entropy: {decode_entropy:.6f}")
    print(f"  Hidden Norm: {decode_hidden_norm:.6f}")
    print("  Top-10 tokens:")
    for idx, (tid, val, tstr) in enumerate(zip(decode_top10_ids, decode_top10_values, decode_top10_tokens)):
        print(f"    [{idx+1}] ID={tid:<6} Logit={val:.4f} Token={repr(tstr)}")

    # Check for NaN / Inf
    no_nan_inf = True
    if np.isnan(decode_entropy) or np.isinf(decode_entropy):
        no_nan_inf = False
    if np.isnan(decode_hidden_norm) or np.isinf(decode_hidden_norm):
        no_nan_inf = False
    ...
    if args.save_golden:
        ...
        golden_path = oracle_dir / "exact_good_1token.json"
        ...
        golden_data = {
            ...
            "tokenizer_ids": ids,
            "expected_decoded_token": decoded_token,
            "expected_top10_ids": decode_top10_ids,
            "expected_entropy": decode_entropy,
            "expected_entropy_range": [decode_entropy - args.entropy_tol, decode_entropy + args.entropy_tol],
            "expected_hidden_norm": decode_hidden_norm,
            "expected_hidden_norm_range": [decode_hidden_norm - args.norm_tol, decode_hidden_norm + args.norm_tol],
            "expected_expert_stats": selected_expert_stats
        }
        
        with open(golden_path, "w") as f:
            json.dump(golden_data, f, indent=2)
        print(f"\nSaved 1-token golden oracle to {golden_path}")
    ...
    if compare_path:
        ...
        overlap = len(set(decode_top10_ids) & set(golden_top10))
        overlap_ok = overlap >= 7

        ent_range = golden.get("expected_entropy_range", [0.0, 0.0])
        ent_ok = ent_range[0] <= decode_entropy <= ent_range[1]

        norm_range = golden.get("expected_hidden_norm_range", [0.0, 0.0])
        norm_ok = norm_range[0] <= decode_hidden_norm <= norm_range[1]

        token_ok = decoded_token == golden.get("expected_decoded_token")

        # Compare expert selections
        gold_experts = golden.get("expected_expert_stats", {})
        expert_match_count = 0
        moe_layers_compared = 0
        for l_key, lyr_stats in selected_expert_stats.items():
            if l_key in gold_experts:
                moe_layers_compared += 1
                curr_ids = set(lyr_stats.get("last_selected_ids", []))
                gold_ids = set(gold_experts[l_key].get("last_selected_ids", []))
                if curr_ids == gold_ids:
                    expert_match_count += 1
        expert_ok = (expert_match_count == moe_layers_compared) if moe_layers_compared > 0 else True

        passed = prompt_mode_ok and overlap_ok and ent_ok and norm_ok and token_ok and expert_ok and no_nan_inf

        print("\nVerification Results:")
        print(f"  0. Prompt Mode:      {'PASS' if prompt_mode_ok else 'FAIL'} (Golden: {golden.get('prompt_mode')} / Current: {prompt_mode})")
        print(f"  1. Decoded Token:    {decoded_token} (Expected: {golden.get('expected_decoded_token')}) -> {'PASS' if token_ok else 'FAIL'}")
        print(f"  2. Top-10 Overlap:   {overlap}/10 (Expected >= 7) -> {'PASS' if overlap_ok else 'FAIL'}")
        print(f"  3. Entropy:          {decode_entropy:.6f} (Expected: {ent_range[0]:.6f} - {ent_range[1]:.6f}) -> {'PASS' if ent_ok else 'FAIL'}")
        print(f"  4. Hidden Norm:      {decode_hidden_norm:.6f} (Expected: {norm_range[0]:.6f} - {norm_range[1]:.6f}) -> {'PASS' if norm_ok else 'FAIL'}")
        print(f"  5. Expert Selection Match: {expert_match_count}/{moe_layers_compared} layers -> {'PASS' if expert_ok else 'FAIL'}")
        print(f"  6. NaN/Inf check:    {'PASS' if no_nan_inf else 'FAIL'}")
        ...
        if not passed:
            if not prompt_mode_ok:
                ...
                failure_type = "Next-token mismatch"
                suspected_area = "Greedy decoding / lm_head logits scaling"
            elif not expert_ok:
                failure_type = "Router expert selection divergence"
                suspected_area = "MoE gating policy / top-k routing"
            elif not no_nan_inf:
                failure_type = "NaN/Inf values"
                suspected_area = "Numerical stability"
            else:
                failure_type = "Metric boundary regression"
                suspected_area = "Rounding differences / precision bits"
        ...
        summary = {
            ...
            "prompt_hash": prompt_hash,
            "prompt_mode": prompt_mode,
            "tok_s": 1.0 / (total_time_ms / 1000.0),
            "total_wall_ms": total_time_ms,
            "forward_wall_ms_avg": total_time_ms / (len(ids) + 1),
            "moe_wall_ms_avg": 0.0,
            "non_moe_wall_ms_avg": total_time_ms / (len(ids) + 1),
            "moe_fraction": 0.0,
            ...
            "entropy_first": decode_entropy,
            "entropy_last": decode_entropy,
            "entropy_min": decode_entropy,
            "entropy_max": decode_entropy,
            
            # Oracle fields
            "pass": passed,
            "decoded_token": decoded_token,
            "entropy": decode_entropy,
            "hidden_norm": decode_hidden_norm,
            "top10_ids": decode_top10_ids,
