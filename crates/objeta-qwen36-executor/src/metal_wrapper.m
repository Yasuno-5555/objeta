...
typedef struct {
    ...
    // Cached pipelines
    id<MTLComputePipelineState> pipe_fp16_gemv, pipe_q4_gemv, pipe_multi_expert, pipe_gqa;
    // GQA persistent buffers (pre-loaded weights + reusable caches)
    id<MTLBuffer> gqa_w, gqa_k, gqa_v, gqa_h, gqa_cos, gqa_sin, gqa_out;
    size_t gqa_kv_cap;
} MetalGpu;
...
void metal_destroy(MetalGpu* gpu) {
    if (gpu) {
        ...
        [gpu->y_buf release];
        [gpu->w_buf release];
        [gpu->pipe_fp16_gemv release];
        [gpu->pipe_q4_gemv release];
...
// ── Preload GQA weights (call once) ─────────────────────────────────────

int metal_gqa_preload(MetalGpu* gpu, const float* w_qkv, size_t w_bytes,
                       const float* rope_cos, const float* rope_sin, uint32_t max_seq) {
    if (!gpu) return -1;
    // Weights: loaded once, never change
    if (!gpu->gqa_w) {
        gpu->gqa_w = [gpu->device newBufferWithBytes:w_qkv length:w_bytes options:MTLResourceStorageModeShared];
        [gpu->gqa_w retain];
    }
    // RoPE tables: also constant
    ...
    // Hidden state buffer (2048 floats, reused)
    if (!gpu->gqa_h) {
...
// ── Dispatch fused GQA (call per token) ──────────────────────────────────

int metal_fused_gqa(
    MetalGpu* gpu,
    const float* w_qkv, size_t w_qkv_bytes,  // ignored if preloaded
    const float* h, const float* rope_cos, const float* rope_sin,
    uint32_t pos, uint32_t seq_len, uint32_t max_seq,
    ...
    float* attn_out)
{
    ...
    [enc setBuffer:gpu->gqa_w   offset:0 atIndex:0];
    [enc setBuffer:gpu->gqa_h   offset:0 atIndex:1];
    [enc setBuffer:gpu->gqa_k   offset:0 atIndex:2];
    [enc setBuffer:gpu->gqa_v   offset:0 atIndex:3];
    [enc setBuffer:gpu->gqa_cos offset:0 atIndex:4];
    [enc setBuffer:gpu->gqa_sin offset:0 atIndex:5];
    [enc setBytes:&pos     length:4 atIndex:6];
    [enc setBytes:&seq_len length:4 atIndex:7];
    [enc setBytes:&max_seq length:4 atIndex:8];
    [enc setBuffer:gpu->gqa_out offset:0 atIndex:9];
