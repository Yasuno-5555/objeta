...
fn f16_to_f32(h: u16) -> f32 {
    ...
    if exp == 0 {
        if mantissa == 0 {
            ...
            (mantissa as f32) * 2f32.powi(1 - 15 - 10) * if sign == 0 { 1.0 } else { -1.0 }
        }
    } else if exp == 31 {
        if mantissa == 0 {
...
pub fn gqa_attention(
    ...
    for h in 0..n_heads {
        ...
        for t in 0..seq_len {
            let mut dot = 0.0f32;
            let k_t =
                &k_cache[(kv_h * seq_len + t) * head_dim..(kv_h * seq_len + t + 1) * head_dim];
            for d in 0..head_dim {
                dot += q_h[d] * k_t[d];
            }
            scores[t] = dot * scale;
        ...
        for t in 0..seq_len {
            let v_t =
                &v_cache[(kv_h * seq_len + t) * head_dim..(kv_h * seq_len + t + 1) * head_dim];
            let a = scores[t];
            for d in 0..head_dim {
                out_h[d] += a * v_t[d];
            }
...
#[no_mangle]
pub extern "C" fn lko_rms_norm(x: *mut c_float, weight: *const c_float, n: c_int) -> c_int {
    ...
    unsafe {
        std::ptr::copy_nonoverlapping(result.as_ptr(), x, n as usize);
    }
    n
}
...
pub extern "C" fn lko_gqa_attention(
    q: *const c_float,
    k_cache: *const c_float,
    v_cache: *const c_float,
    n_heads: c_int,
    n_kv: c_int,
    head_dim: c_int,
    seq_len: c_int,
    ...
    let k_slice =
        unsafe { std::slice::from_raw_parts(k_cache, (n_kv * seq_len * head_dim) as usize) };
    let v_slice =
        unsafe { std::slice::from_raw_parts(v_cache, (n_kv * seq_len * head_dim) as usize) };
...
pub extern "C" fn lko_silu(x: *const c_float, y: *mut c_float, n: c_int) -> c_int {
    ...
    let result = silu(X);
    unsafe {
        std::ptr::copy_nonoverlapping(result.as_ptr(), y, n as usize);
    }
    n
}
