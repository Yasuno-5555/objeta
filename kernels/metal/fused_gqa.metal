// objeta — Fused GQA Attention Metal Kernel (stepwise debug)
//
// Stage 1: QKV projection only — validate indexing
...
constant uint Q_SZ  = 8192;
constant uint K_SZ  = 512;

kernel void fused_gqa(
    device const float* W_qkv   [[buffer(0)]],
...
)
{
    ...
    for (uint j = 0; j < D; j++) {
        float hj = h[j];
        q_val      += W_qkv[q_row  * D + j] * hj;
        q_gate_val += W_qkv[qg_row * D + j] * hj;
    }

    uint k_row = Q_SZ + kv_head * HD + tid;
    float k_val = 0.0;
    for (uint j = 0; j < D; j++) { k_val += W_qkv[k_row * D + j] * h[j]; }

    uint v_row = Q_SZ + K_SZ + kv_head * HD + tid;
    float v_val = 0.0;
    for (uint j = 0; j < D; j++) { v_val += W_qkv[v_row * D + j] * h[j]; }
    ...
    if (tid < hd2) {
        float q_even = q_val;
        float q_odd  = tg[tid + hd2];
        q_rot = q_even * cos_v - q_odd * sin_v;
    } else {
        float q_odd  = q_val;
        float q_even = tg[tid - hd2];
        q_rot = q_even * sin_v + q_odd * cos_v;
    }

    // ── RoPE for K: share K values via tg ──
    tg[tid] = k_val;
    threadgroup_barrier(mem_flags::mem_threadgroup);
    ...
    if (tid < hd2) {
        float k_even = k_val;
        float k_odd  = tg[tid + hd2];
        k_rot = k_even * cos_v - k_odd * sin_v;
    } else {
        float k_odd  = k_val;
        float k_even = tg[tid - hd2];
        k_rot = k_even * sin_v + k_odd * cos_v;
    }
    ...
    for (uint t = 0; t < seq_len; t++) {
        // Partial dot product: q_rot[dim] * k[t, dim]
        float k_td = k_cache[kv_head * max_seq * HD + t * HD + tid];
        tg[tid] = q_rot * k_td;
        threadgroup_barrier(mem_flags::mem_threadgroup);

        // Thread 0 reduces full dot product
        if (tid == 0) {
            float score = 0.0f;
            for (uint d = 0; d < HD; d++) score += tg[d];
            tg[HD] = score * scale;  // store full score
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
