#!/usr/bin/env python3
"""Qwen3.6-35B-A3B — pure numpy + Rust MoE dispatch. Zero MLX.

Key optimizations:
- All matmuls via NumPy @ (Apple Accelerate)
- QKV weights pre-concatenated (1 matmul instead of 3)
- gate+up weights pre-concatenated for shared expert
- Only Rust FFI for MoE dispatch (q4 dequantize + GEMV)
"""
...
class Qwen36NumpyGen:
    def __init__(self):
        ...
        print("Loading + concatenating weights...", end=" ", flush=True)
        self._w = {}  # layer → concatenated weight dict
        for l in range(40):
            a = AttnWeightLoader(l)
            w = {}
            # Cache all individual weights
            for key in a.meta:
                if key != '__metadata__':
                    w[key] = a.get(key)

            # ★ QKV concatenation for full attention layers
            if l % 4 == 3:
                # q_proj: (4096+256, 2048), k_proj: (512, 2048), v_proj: (512, 2048)
                # → W_qkv: (4352+512+512, 2048) = (5376, 2048)
                ...
                w['_q_sz'] = q_w.shape[0]  # 4352 (= 4096 q + 256 gate)
                w['_k_sz'] = k_w.shape[0]  # 512
                w['_n_heads'] = N_HEADS
                w['_n_kv'] = N_KV
                w['_hd'] = HEAD_DIM

            # ★ gate+up concatenation for shared expert
            if a.has('mlp.shared_expert.gate_proj.weight'):
                g_w = w['mlp.shared_expert.gate_proj.weight']  # (512, 2048)
                u_w = w['mlp.shared_expert.up_proj.weight']    # (512, 2048)
                w['_gateup_cat'] = np.concatenate([g_w, u_w], axis=0)  # (1024, 2048)

            # Conv1d weight reshape
    ...
    # ── Full GQA Attention (numpy, fused QKV) ───────────────────────────

    def _full_attn(self, h, l, pos, seq_len, kv):
        w = self._w[l]
        W_qkv = w['_qkv_cat']
        q_sz, k_sz = w['_q_sz'], w['_k_sz']
        ...
        return w['self_attn.o_proj.weight'] @ (attn_out * q_gate), (Kc, Vc)

    # ── GatedDeltaNet (numpy) ──────────────────────────────────────────

    def _delta_net(self, h, l, state):
        w = self._w[l]
        mixed_qkv = w['linear_attn.in_proj_qkv.weight'] @ h
        ...
        return w['linear_attn.out_proj.weight'] @ gated.reshape(-1)

    # ── Shared Expert (numpy, fused gate+up) ───────────────────────────

    def _shared_expert(self, h, l):
        w = self._w[l]
        if '_gateup_cat' not in w:
            return np.zeros(HDIM, dtype=np.float32)

        gu_w = w['_gateup_cat']  # (1024, 2048) = gate(512) + up(512)
        gate_out = gu_w[:512] @ h
        up_out = gu_w[512:] @ h
        hidden = gate_out / (1.0 + np.exp(-gate_out)) * up_out
        se_raw = w['mlp.shared_expert.down_proj.weight'] @ hidden
        se_gate = 1.0 / (1.0 + np.exp(-(w['mlp.shared_expert_gate.weight'].flatten() @ h)))
        return se_raw * se_gate
    ...
    def generate(self, prompt_ids, max_tokens=20, temperature=0.7, top_k=40):
        # Init KV caches + DeltaNet states
...
def main():
    print("=== Qwen3.6-35B-A3B (NumPy + Rust MoE) ===\n")
    gen = Qwen36NumpyGen()
